...
async def load_income_entries(user_id: str, access_token: str, limit: int = 12) -> list[dict]:
    try:
...
async def save_income_entry(user_id: str, data: dict, access_token: str) -> Optional[dict]:
    try:
        client = get_user_client(access_token)
...
async def delete_income_entry(user_id: str, month: int, year: int, access_token: str) -> bool:
    try:
        client = get_user_client(access_token)
